...
exports.expires = function(test) {
  ...
  limiter.use('ttl')
    .then(function(res) {
      redisClient.pttl(prefix + ':ttl:V', function(err, res) {
        if (err) {
...
exports.useMultiple = function(test) {
  test.expect(2);
  defaultLimiter.use('useMultiple', 3)
    .then(function(res) {
      test.equal(res.remaining, 0);
      test.equal(res.rejected, false);
      test.done();
